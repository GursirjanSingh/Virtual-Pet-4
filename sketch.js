...
//load images here
 ...
 hungryDog = loadImage("images/dogIngl.png");

 happyDog =loadImage("images/dogIng.png"); 
  bedroonIMG=loadImage("images/Bed Room.png");
...
runIMG= loadImage("images/running.png");
 ...
 dog =  createSprite(width/2+250,height/2,10,10);
  ...
  feed. position (958,95);
  feed.mousePressed(feedDog);
 
 
  addFood= createButton("Add food"); 
   addFood.position (1850,95);
  addFood.mousePressed(addFoods);

  input = createInput("Pet name");
input.position (958,120);
...
addFood.show();
 ...
 textSize(32); fill("red");
  ...
  text("Time since last fed:" +(currentTime- lastFed), 308,125) 
 }
function feedDog(){
 ...
 function addFoods() {
  ...
  var site = await fetch("http://worldtimeapi.org/api/timezone/America/N 127 var siteJSON-await site.json()");
